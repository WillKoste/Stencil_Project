...
export default class Cart extends PageManager {
	...
	addLineItem() {
		...
		axios
			.post(`http://localhost:3000/api/storefront/carts/${this.context.cartId}`, JSON.stringify(body), config)
			.then(() => {
				swal
					.fire({
						text: 'All items have been added'
					})
					.then(() => window.location.reload());
				// .then(() => itemDeleted && window.location.reload());
			})
			.catch((err) => {
				console.log({err});
				if (err.response.status === 422) {
					swal.fire({text: 'Item is already in your cart'});
				}
			});
	}
	...
	deleteCart(cartId) {
		try {
			axios
				.delete(`http://localhost:3000/api/storefront/carts/${cartId}`)
				.then(() => swal.fire({text: 'Your cart has been cleared'}))
